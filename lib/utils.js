...
var Utils = {
  ...
  createSearchQuery(query, searchUrl) {
    if (typeof(query) === "string") { query = query.split(/\s+/); }
    if (searchUrl.match(/^https:\/\/scrapbox\.io/)) {
      return query.map(encodeURIComponent).join("_")
    } else {
  ...
  createSearchUrl(query, searchUrl) {
    ...
    return searchUrl.replace(/%s/g, this.createSearchQuery(query, searchUrl));
  },
